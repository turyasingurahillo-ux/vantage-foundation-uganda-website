...
- `INBOUND_EMAIL_SECRET` — bearer token required by the `/api/inbound/email` endpoint. If unset, the endpoint fails closed (503). Used by the Cloudflare Email Worker to authenticate inbound reply delivery. Rotate to revoke the worker's access.
...
## Inbound email
Inbound email replies to outbound Vantage correspondence are processed through an authenticated endpoint. The email transport (Cloudflare Email Routing / Email Workers) is configured separately and POSTs parsed email data to the application endpoint.

**Endpoint**: `POST /api/inbound/email`
**Authentication**: Bearer token via `INBOUND_EMAIL_SECRET`. If unset, the endpoint fails closed (503). The token is checked with `safeSecretEqual` (constant-time comparison) to prevent timing attacks.

**Operational constraints**:
- **Replay protection**: each inbound email is hashed (SHA-256 of Message-ID + sender + date, truncated to 32 hex chars) and stored in `inbound_email_log` with a `UNIQUE` constraint. A replay of the same email is silently rejected (`replay_blocked`).
- **Thread matching**: the `In-Reply-To` and `References` headers are matched against `contact_message_replies.provider_message_id` (outbound, `send_status = 'sent'`). An email with no threading headers, or whose headers do not match any outbound reply, is logged as `unmatched` — no case is created or modified.
- **No arbitrary case-ID injection**: the case is resolved exclusively from the reply thread. The endpoint accepts no case ID in the request body.
- **Workflow update**: when a match is found, an inbound reply row is created in `contact_message_replies` (direction = `inbound`), and the case `workflow_status` is set to `awaiting_vantage` with `status` reset to `awaiting_response`. This surfaces the case for admin attention without completing it.
- **Size limits**: body is truncated to 100 KB before storage. The Zod schema rejects bodies larger than 200 KB.
- **Rate limiting**: 60 requests per minute per IP.
- **No secrets in responses**: the endpoint returns only `{ status, caseId }` — no sender addresses, subjects, or internal IDs beyond the matched case.

**Key files**:
- `app/api/inbound/email/route.ts` — authenticated endpoint (Zod validation, bearer token, rate limit).
- `lib/db/inbound-email.ts` — application logic (hash, replay check, thread match, reply row creation, workflow update, logging).
- `lib/db/migrations/organisation-relationship-pipeline.sql` — `inbound_email_log` table schema (replay protection + audit trail).
- `lib/safe-compare.ts` — `parseBearerToken`, `safeSecretEqual` (constant-time comparison).
- `lib/rate-limit.ts` — per-IP rate limiting.

**Database tables**:
- `inbound_email_log` — replay protection and audit trail. Columns: `message_id_hash` (UNIQUE), `from_address`, `in_reply_to`, `subject`, `matched_case_id`, `matched_reply_id`, `status` (`processed` / `unmatched` / `replay_blocked` / `error`), `error_detail`.
- `contact_message_replies` — inbound replies are stored here with `direction = 'inbound'`, alongside outbound replies (`direction = 'outbound'`).

**Not implemented**: the Cloudflare Email Worker that parses raw email and POSTs to this endpoint is configured outside this repository. The endpoint is ready to receive once the worker is deployed.
