...
import { trackWhatsAppClick } from "@/components/shared/WhatsAppClickTracker";
...
interface WhatsAppButtonClientProps {
  ...
  message?: string;
  /** Where the CTA appears, for analytics. e.g. "contact page", "footer". */
  context: string;
  /** Optional position label, e.g. "hero", "card". */
  position?: string;
  /** Visual variant. */
...
/**
 * Client-side WhatsApp button with analytics tracking.
 *
 * Same visual as the server WhatsAppButton, but fires a
 * `whatsapp_contact_click` event on click before navigating. Use this
 * version when analytics tracking is needed (most public CTAs).
 */
export function WhatsAppButtonClient({
  ...
  return (
    <a
      ...
      onClick={() => {
        trackWhatsAppClick({ context, destination: href, position });
      }}
      className={cn(
